                ...
                if not "scoreBreakdown" in matchInDB["results"]:
                    matches.update_one(
                        {"matchKey": match["key"]},
                        {
                            "$set": {
                                "results.scored": True,
                                "results.postResultTime": match["post_result_time"],
                                "results.actualTime": match["actual_time"],
                                "results.scoreBreakdown": match["score_breakdown"],
                                "results.winningAlliance": match["winning_alliance"],
                            }
                        },
                    )
                    app.logger.info(
...
def saveAlliancesFromTBA(event: str):
    ...
    try:
        data = requests.get(
            f"https://www.thebluealliance.com/api/v3/event/{event}/alliances",
            headers={"X-TBA-Auth-Key": TBA_KEY, "User-Agent": "Nerd Scout"},
        )
        data = json.loads(data.text)
        if "Error" in data:
...
def updateMatchFromTBA(compLevel: CompLevel, matchNumber: int, setNumber: int) -> bool:
    ...
    if not matchDataList:
        ...
        app.logger.error(
            f"Error updating scoring data: multiple matches found for {compLevel.value}{matchNumber}, set {setNumber}."
        )
        ...
        if not "scoreBreakdown" in matchData["results"]:
            matches.update_one(
                {"matchKey": TBAdata["key"]},
                {
                    "$set": {
                        "results.scored": True,
                        "results.postResultTime": TBAdata["post_result_time"],
                        "results.actualTime": TBAdata["actual_time"],
                        "results.scoreBreakdown": TBAdata["score_breakdown"],
                        "results.winningAlliance": TBAdata["winning_alliance"],
                    }
                },
            )
            app.logger.info(
...
def createPrediction(
    ...
    accounts.update_one(
        ...
        {
            "$set": {
                f"predictions.{matchData['matchKey']}": {
                    "forRed": forRed,
                    "points": points,
                    "matchComplete": False,
                    "correct": False,
    ...
    matches.update_one(
        ...
        {
            "$inc": {
                "prizePool.overall": points,
                f"prizePool.{'red' if forRed else 'blue'}": points,
            }
        },
    )
    app.logger.info(
        f"Created prediction for {user} on {compLevel}{matchNumber}_{setNumber}: {points} for {'Red' if forRed else 'Blue'} Victory."
    )
...
def payoutPredictions(matchKey: str, forRed: bool) -> None:
    ...
    for user in correctUsers:
        ...
        accounts.update_one(
            ...
            {
                "$inc": {"points": payout},
                "$set": {f"predictions.{matchKey}.correct": True},
            },
        )
...
def createPickEms(
    ...
    updateStatus = accounts.update_one(
        {"username": user}, {"$set": {"pickems": bracket}, "$inc": {"points": -points}}
    ).acknowledged
    if updateStatus:
...
def payPickEms() -> bool:
    ...
    if finals1Winner == finals2Winner:
        ...
        app.logger.error(
            "Failed to pay out pickems: finals3 winner not found and finals not clinched."
        )
        return False
    else:
    ...
    for user in users:
        ...
        for pick in round1Picks:
            ...
            if userPickems[pick][userPickems[pick]["winner"]] == realWinningAlliance:
                app.logger.info(
                    f"Correct pick for {realWinningAlliance} in {pick} for {user['username']}"
                )
        ...
        if userPickems["winner"] == realFinalsWinner:
            ...
            app.logger.info(
                f"Correct pick for {realFinalsWinner} in finals for {user['username']}"
            )

        updateStatus = accounts.update_one(
            {"username": user["username"]},
            {"$inc": {"points": points}, "$set": {"pickems.paid": True}},
        ).acknowledged
        if updateStatus:
...
def createTestTBABreakdown() -> dict:
    ...
    scoreBreakdown = {
        ...
        "hubScore": {
            ...
            "teleopCount": transitionCount
            + shift1Count
            + shift2Count
            + shift3Count
            + shift4Count
            + endgameCount,
            "teleopPoints": transitionCount
            + (shift1Count if firstShift else shift2Count)
            + (shift3Count if firstShift else shift4Count)
            + endgameCount,
